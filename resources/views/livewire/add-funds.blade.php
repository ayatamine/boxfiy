<div class="container">
            ...
            <form    wire:submit.prevent="submitAddFund">
                ...
                <div class="funds-content">   
                    <div class="incrementer">
                        <button type="button" class="plus-btn" wire:click.prevent="incrementAmount"><img class="mx-auto" src="{{asset('BoxfiyV6/images/icons/14.png')}}"></button>
                        <div class="price">
                            <input type="number" step="0.00" min="{{$min_amount}}" max="{{$max_amount}}" wire:model.defer="amount" name="amount" >
                        </div>
        
                        <button type="button" class="minus-btn" wire:click.prevent="decrementAmount"><img class="mx-auto"  src="{{asset('BoxfiyV6/images/icons/15.png')}}"> </button>
                    </div>
                    ...
                    @endforeach    
                    @if(count($payment_gateways) && (count($payment_gateways) % 2 != 0))
                    <label id="pay{{count($payment_gateways)+1}}" for="">
    ...
    <form  class="mt-5 hidden " id="spaceremit-form" action="https://spaceremit.com/apipay/" method="POST" accept-charset="utf-8">
        @csrf
        <input type="hidden" name="cmd" value="_xclick">
        <input type="hidden" name="business" value="{{ $spaceremit_email }}">
        <input type="hidden" name="currency_code" value="USD">
        ...
        <div class="mt-4">
            <label for="amount_to_pay" class="block text-lg text-gray-700 capitalize dark:text-gray-200">Amount to charge</label>
            <input name="amount" id="amount_to_pay" placeholder="0.00" type="number" wire:model="final_amount" step="0.00" class="block w-full px-3 py-2 mt-2 text-gray-600 placeholder-gray-400 bg-white border border-gray-200 rounded-md focus:border-indigo-400 focus:outline-none focus:ring focus:ring-indigo-300 focus:ring-opacity-40">
        </div>
